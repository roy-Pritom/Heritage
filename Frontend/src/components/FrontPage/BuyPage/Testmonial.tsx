"use client";
import { useGetAllReviewQuery, useGetReviewQuery } from "@/components/Redux/api/reviewApi";
import { Star } from "lucide-react";
import Image from "next/image";
import React, { useState } from "react";

const Testimonial = ({propertyId,isAllReview}:{propertyId:string,isAllReview:boolean}) => {
  // console.log({propertyId});
  const {data:reviewsByProperty,isLoading:reviewLoading}=useGetReviewQuery(propertyId);
    ...
    <div className="container mx-auto py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">

        {
          isAllReview ?
          <>
                  {reviews?.data?.slice(activeIndex * 2, activeIndex * 2 + 2)
          .map((testimonial:any) => (
            <div
              key={testimonial._id}
            ...
            >
              ...
              <h2 className="text-xl font-semibold text-slate-800">
                {testimonial?.reviewerId?.firstName} {testimonial?.reviewerId?.lastName}
              </h2>
              <h3 className="text-sm text-slate-600">{testimonial?.reviewerId?.email}</h3>
            </div>
          ))}
          ...
          :
          <>
          {reviewsByProperty?.data?.length===0 && <p className="text-xl font-bold text-center">Reviews : 0</p>}
                  {reviewsByProperty?.data?.slice(activeIndex * 2, activeIndex * 2 + 2)
          .map((testimonial:any) => (
            <div
              key={testimonial._id}
              className="bg-[#ecf5ff] p-6 rounded-lg flex flex-col items-center"
